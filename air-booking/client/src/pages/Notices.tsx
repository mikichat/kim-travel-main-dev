...
export function Notices() {
  ...
  return (
    <div style={{ padding: '0 4px' }}>
      ...
      {items.length === 0 ? (
        <div style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>등록된 안내문이 없습니다.</div>
      ) : (
            ...
            return (
              <div key={item.id} style={{ border: '1px solid #e2e8f0', borderRadius: '8px', padding: '12px 16px', background: '#fff' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <strong>{(d.title as string) || '안내문'}</strong>
      ...
      {(editId || creating) && (
        <Modal open={true} title={editId ? '안내문 편집' : '안내문 등록'} onClose={() => { setEditId(null); setCreating(false); }}>
